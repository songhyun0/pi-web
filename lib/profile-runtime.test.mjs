...
test("isolated runtime resolves dynamic selected-package tools before snapshot persistence", async () => {
  ...
  provisional.diagnostics = [
    {
      type: "warning",
      message: "Enabled extension tool metadata will be resolved from the isolated session runtime.",
      source: "selected-pkg",
      path: "extensions/dynamic.mjs",
    },
    { type: "warning", message: "Keep this diagnostic.", source: "selected-pkg" },
  ];
...
test("standalone skill identity follows canonical path when its frontmatter name changes", async () => {
  const { validateProfileRuntimeAgainstSnapshot } = await loadRuntime();
  const skillPath = "/agent/skills/style-seed/SKILL.md";
  const loadedSkill = {
    name: "styleseed",
    filePath: skillPath,
    sourceInfo: { source: "auto", path: skillPath, origin: "top-level", scope: "user", baseDir: "/agent" },
  };
  const fakeSession = {
    getAllTools: () => [],
    getActiveToolNames: () => [],
  ...
  const snapshot = sampleSnapshot({
    requestedBuiltinTools: [],
    activeToolNames: [],
    visibleSkillRefs: [{ source: skillPath, scope: "user", path: skillPath, name: "style-seed" }],
  });
...
test("profile binding reconciles setup mutations before enforcing later violations", async () => {
  ...
  let activeDuringBinding;
  const fakeInner = {
    sessionId: "rpc-profile-binding-window-test",
    sessionFile: undefined,
    isStreaming: false,
    isCompacting: false,
    isBashRunning: false,
    ...
    async bindExtensions() {
      this.setActiveToolsByName(["bash"]);
      activeDuringBinding = [...this.active];
    },
  };
  const wrapper = new AgentSessionWrapper(fakeInner, sampleSnapshot({ requestedBuiltinTools: ["read"], activeToolNames: ["read"] }));

  await wrapper.bindExtensions();
  assert.deepEqual(activeDuringBinding, []);
  assert.deepEqual(fakeInner.active, ["read"]);
  assert.equal(wrapper.isAlive(), true);

  try { fakeInner.setActiveToolsByName(["bash"]); } catch { /* Pi may report handler errors and continue. */ }
  assert.equal(wrapper.isAlive(), false);
