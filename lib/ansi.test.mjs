...
test("parses ANSI underline and strikethrough decorations", async () => {
  const { parseAnsiLine } = await loadSubject();

  assert.deepEqual(parseAnsiLine("\x1b[4;9mtask\x1b[29m under\x1b[24m plain"), [
    { text: "task", style: { textDecoration: "underline line-through" } },
    { text: " under", style: { textDecoration: "underline" } },
    { text: " plain", style: {} },
  ]);
});
