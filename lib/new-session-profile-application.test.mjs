...
test("persists runtime-resolved plugin tools finalized after extension binding", async () => {
  const { createProfileBackedNewSessionRuntime } = await loadApplication();
  const events = [];
  const runtime = makeRuntime(events, {
    allTools: [
      { name: "read", sourceInfo: { source: "builtin", path: "<builtin:read>" } },
      { name: "dynamic_tool", sourceInfo: { source: "pkg", path: "/pkg/extensions/dynamic.mjs", origin: "package" } },
    ],
  });
  let provisionalSnapshot;
  runtime.session.finalizeProfileToolPolicy = () => {
    events.push("finalize-tools");
    return {
      ...provisionalSnapshot,
      tools: {
        ...provisionalSnapshot.tools,
        pluginTools: [{ name: "dynamic_tool", source: "pkg", extension: "/pkg/extensions/dynamic.mjs", provenance: "plugin", metadataResolved: true }],
        activeToolNames: ["read", "dynamic_tool"],
        conflicts: [],
      },
    };
  };
  const preview = makePreview({ unknownToolMetadata: true });
  preview.plugins = [{ source: "pkg", prompts: [], themes: [] }];
  const dependencies = makeBaseDependencies(events, {
    runtime,
    preview,
    onPersist: ({ snapshot }) => assert.deepEqual(snapshot.tools.pluginTools.map((tool) => tool.name), ["dynamic_tool"]),
  });
  const baseCreate = dependencies.createUnregisteredRpcSession;
  dependencies.createUnregisteredRpcSession = async (...args) => {
    provisionalSnapshot = args[2].profileSnapshot;
  ...
  assert.deepEqual(result.profileSnapshot.tools.activeToolNames, ["read", "dynamic_tool"]);
  assert.ok(events.indexOf("bind") < events.indexOf("finalize-tools"));
  assert.ok(events.indexOf("finalize-tools") < events.indexOf("persist"));
});
