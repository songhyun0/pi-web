...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false, throwOnError = false) => {
    ...
    try {
      ...
      if (requestEpoch !== runtimeEpochRef.current || sessionIdRef.current !== sid) return null;
      setData(d);
      ...
      if (showLoading && !messagesLoaded && requestEpoch === runtimeEpochRef.current) setLoading(false);
    }
